...
const PohonNodeOpd: React.FC<PohonNodeOpdProps> = ({ node, onTreeRefresh, forceExpand }) => {
        ...
        <div className="bg-white p-2 rounded-b-lg">
          <table className="w-full border-collapse text-xs">
            <tbody>
              ...
              {/* Baris 2 & 3: INDIKATOR & TARGET */}
              {node.indikator.length > 0 ? (
                node.indikator.map((ind: Indikator, idx: number) => (
                  <React.Fragment key={ind.id}>
                    <tr>
                      <td className="border p-2 font-semibold text-gray-600 w-24">
                        Indikator {node.indikator.length > 1 ? idx + 1 : ''}
                      </td>
              ...
              {/* Baris 4: KETERANGAN (POHON) */}
              <tr>
                <td className="border p-2 font-semibold text-gray-600">Keterangan</td>
                <td className="border p-2 text-gray-700 bg-gray-50">
                  {node.keterangan || "-"}
                </td>
              </tr>
            </tbody>

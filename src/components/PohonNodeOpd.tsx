...
interface PohonNodeProps {
  ...
  isRoot?: boolean;
  // Props tambahan untuk OPD
  parentKodeOpd?: string;
  parentKodePemda?: string;
}

// --- INTERNAL COMPONENT: Form Edit ---
const FormEditNode = ({ node, parentKodeOpd, parentKodePemda, onCancel, onSuccess }: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const activeOpd = opd?.value || node.kodeOpd || parentKodeOpd;
      ...
      const payload = {
        ...
        kodePemda: String(node.kodePemda || parentKodePemda || ""),
...
// --- INTERNAL COMPONENT: Form Add ---
const FormAddChildModal = ({ parentId, childInfo, kodeOpd, kodePemda, onCancel, onSuccess }: any) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        const activeOpd = opd?.value || kodeOpd;
...
const getHeaderStyle = (jenisPohon: string) => {
  switch (jenisPohon) {
    case "STRATEGIC_OPD": return "border-red-700 text-white bg-gradient-to-r from-[#CA3636] from-40% to-[#BD04A1]";
    case "TACTICAL_OPD": return "border-blue-500 text-white bg-gradient-to-r from-[#3673CA] from-40% to-[#08D2FB]";
    case "OPERATIONAL_OPD": return "border-green-500 text-white bg-gradient-to-r from-[#139052] from-40% to-[#2DCB06]";
    default: return "border-black bg-white text-black";
  }
};

const getButtonColor = (jenisPohon: string) => {
  if (jenisPohon === 'STRATEGIC_OPD') return 'border-[#3072D6] text-[#3072D6] hover:bg-[#3072D6]';
  return 'border-[#00A607] text-[#00A607] hover:bg-[#00A607]';
};

const PohonNodeOpd: React.FC<PohonNodeProps> = ({ node, onTreeRefresh, onDeleteAction, isRoot = false, parentKodeOpd, parentKodePemda }) => {
  const styles = getPohonStyle(node.levelPohon);
  const childInfo = getChildInfo(node.levelPohon);
  const hasChildren = node.children && node.children.length > 0;
  
  const currentKodeOpd = node.kodeOpd || parentKodeOpd;
  const currentKodePemda = node.kodePemda || parentKodePemda;
    ...
    <li>
      {isEditing ? (
        <div className="tf-nc" style={{ padding: 0, border: 'none', background: 'transparent' }}>
          <FormEditNode node={node} parentKodeOpd={currentKodeOpd} parentKodePemda={currentKodePemda} onCancel={() => setIsEditing(false)} onSuccess={() => { setIsEditing(false); if (onTreeRefresh) onTreeRefresh(); else window.location.reload(); }} />
        </div>
      ) : (
      ...
      {(isExpanded || isAddModalOpen) && ((hasChildren) || isAddModalOpen) && (
        <ul className="flex justify-center relative">
          {isExpanded && hasChildren && node.children?.map((child) => (
            <PohonNodeOpd key={child.id} node={child} onTreeRefresh={onTreeRefresh} onDeleteAction={onDeleteAction} isRoot={false} parentKodeOpd={currentKodeOpd} parentKodePemda={currentKodePemda} />
          ))}
          {isAddModalOpen && childInfo && (
            <li>
              <div className="tf-nc" style={{ padding: 0, border: 'none', background: 'transparent' }}>
                <FormAddChildModal parentId={node.id} childInfo={childInfo} kodeOpd={currentKodeOpd} kodePemda={currentKodePemda} onCancel={() => setIsAddModalOpen(false)} onSuccess={() => { setIsAddModalOpen(false); if (onTreeRefresh) onTreeRefresh(); else window.location.reload(); }} />
              </div>
            </li>
          )}
        </ul>
